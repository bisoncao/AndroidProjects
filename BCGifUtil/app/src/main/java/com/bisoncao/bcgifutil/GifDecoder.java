...
public class GifDecoder {
    ...
    /**
     * @author Bison Cao
     * recycle the allocated bitmap resources
     */
    public void recycle() {
        if (frames != null) {
            ...
            if (size > 0) {
                try {
                    for (int i = 0; i < size; i++) {
                        frames.elementAt(i).image.recycle();
                    }
                } catch (Exception e) {
                    BCDebug.e(BCDebug.BISON,
                            "error when in GifDecoder's recycle()");
                    BCDebug.printStackTrace(e);
                }
            }
        }
